...
df_signal = uproot.open(
    "/mnt/harddisk4/scratch/ttbar_no_dtG_flat.root")["outtree"].pandas.df(input_branches)
df_dyjets = uproot.open(
    "/mnt/harddisk4/scratch/dyjets_flat_30M.root")["outtree"].pandas.df(input_branches)
df_wjets = uproot.open(
    "/mnt/harddisk4/scratch/wjets_flat_60M.root")["outtree"].pandas.df(input_branches)
df_single_top = uproot.open(
    "/mnt/harddisk4/scratch/single_top_merged_flat.root")["outtree"].pandas.df(input_branches)
...
x_single_top = x_single_top[np.random.randint(
    len(x_single_top), size=len(x_wjets))]
...
idx_dyjets = np.split(
    idx_dyjets, [int(f1*len(idx_dyjets)), int(f2*len(idx_dyjets))])
...
idx_wjets = np.split(
    idx_wjets, [int(f1*len(idx_wjets)), int(f2*len(idx_wjets))])
...
idx_single_top = np.split(idx_single_top, [int(
    f1*len(idx_single_top)), int(f2*len(idx_single_top))])
idx_single_top_train, idx_single_top_val, idx_single_top_test = idx_single_top[
    0], idx_single_top[1], idx_single_top[2]
...
print("number of single_top events for the training:%d" %
      (len(x_single_top_train)))


x_train = np.concatenate(
    (x_sig_train, x_dyjets_train, x_wjets_train, x_single_top_train), axis=0)
y_train = np.concatenate(
    (y_sig_train, y_dyjets_train, y_wjets_train, y_single_top_train), axis=0)
x_train, y_train = shuffle(x_train, y_train, random_state=0)

x_val = np.concatenate(
    (x_sig_val, x_dyjets_val, x_wjets_val, x_single_top_val), axis=0)
y_val = np.concatenate(
    (y_sig_val, y_dyjets_val, y_wjets_val, y_single_top_val), axis=0)
x_val, y_val = shuffle(x_val, y_val, random_state=0)

x_test = np.concatenate(
    (x_sig_test, x_dyjets_test, x_wjets_test, x_single_top_test), axis=0)
y_test = np.concatenate(
    (y_sig_test, y_dyjets_test, y_wjets_test, y_single_top_test), axis=0)
...
print("training for", len(x_train), "events;",
      "testing for",  len(x_val), "events")
...
param_grid = {
    ...
    'optimizer': ['Adam', 'SGD'],
    'losses': ['binary_crossentropy', 'logcosh', 'mse'],
...
print("training for all possible " +
      str(len(permutations_dicts)) + " hyperparameter sets.")
history_arr = []
...
for i, param_set in enumerate(permutations_dicts):
    ...
    history, delta_t = dnn_model(
        x_train, y_train, x_val, y_val, callbacks, param_set)
